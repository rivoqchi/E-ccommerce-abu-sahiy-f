...
export default function AdminHamkorPartnersPage() {
  ...
  const load = useCallback(async () => {
    const data = await adminFetch<Partner[]>("/hamkor/partners?all=true");
    setItems(data);
  }, [adminFetch]);
  ...
  function openEdit(p: Partner) {
    setEditingId(p._id);
    setForm({ name: p.name, image: p.image ?? "" });
    setError(null);
    setOpen(true);
  }
  ...
  function save() {
    ...
    startTransition(async () => {
      try {
        ...
        if (editingId) {
          await adminFetch(`/hamkor/partners/${editingId}`, {
            method: "PATCH",
            body: JSON.stringify(payload),
          });
        } else {
          await adminFetch("/hamkor/partners", {
            method: "POST",
            body: JSON.stringify(payload),
          });
        }
  ...
  async function remove(id: string) {
    try {
      await adminFetch(`/hamkor/partners/${id}`, { method: "DELETE" });
      await load();
    } catch (e) {
      setError(e instanceof Error ? e.message : "Xato");
      throw e;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Hamkorlar</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Avval hamkor nomini qoʻshing, keyin kategoriya va mahsulot bogʻlang.
          </p>
        </div>
                  ...
                  <TableCell className="pr-5 text-right">
                    <div className="inline-flex items-center gap-1">
                      ...
                      <ConfirmAction
                        ...
                        title="Hamkorni oʻchirasizmi?"
                        description={`“${p.name}” hamkori oʻchiriladi. Davom etasizmi?`}
