...
const emptyForm = { name: "", image: "", partnerId: "" };

function partnerIdOf(ref?: PartnerRef): string {
  if (!ref) return "";
  return typeof ref === "string" ? ref : ref._id;
}

function partnerNameOf(ref?: PartnerRef, partners: Partner[] = []): string {
  if (ref && typeof ref === "object" && ref.name) return ref.name;
  const id = partnerIdOf(ref);
  return partners.find((p) => p._id === id)?.name ?? "—";
}

export default function AdminHamkorCategoriesPage() {
  ...
  const load = useCallback(async () => {
    const [cats, prs] = await Promise.all([
      adminFetch<Category[]>("/hamkor/categories?all=true"),
      adminFetch<Partner[]>("/hamkor/partners?all=true"),
    ]);
    setItems(cats);
    setPartners(prs);
  }, [adminFetch]);
  ...
  function openEdit(c: Category) {
    ...
    setForm({
      name: c.name,
      image: c.image ?? "",
      partnerId: partnerIdOf(c.partnerId),
    });
    setError(null);
    setOpen(true);
  }
  ...
  function save() {
    if (!form.name.trim() || !form.partnerId) return;
    setError(null);
    startTransition(async () => {
      try {
        ...
        if (editingId) {
          await adminFetch(`/hamkor/categories/${editingId}`, {
            method: "PATCH",
            body: JSON.stringify(payload),
          });
        } else {
          await adminFetch("/hamkor/categories", {
            method: "POST",
            body: JSON.stringify(payload),
          });
        }
  ...
  async function remove(id: string) {
    try {
      await adminFetch(`/hamkor/categories/${id}`, { method: "DELETE" });
      await load();
    } catch (e) {
      setError(e instanceof Error ? e.message : "Xato");
      throw e;
            ...
            <TableBody>
              {items.map((c) => (
                <TableRow key={c._id}>
                  ...
                  <TableCell>{partnerNameOf(c.partnerId, partners)}</TableCell>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            <Button
              ...
              disabled={
                pending || uploading || !form.name.trim() || !form.partnerId
              }
              onClick={save}
            >
